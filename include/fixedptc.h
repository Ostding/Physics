...
namespace physics
{
	...
	#endif
			const FixedPt operator+(const FixedPt &other) const { return fixedpt_add( this->value, other.value);}
			const FixedPt operator+(const fixedpt other) 	const { return fixedpt_add( this->value, other);}
			const FixedPt operator+(const double other) 	const { return fixedpt_add( this->value, fixedpt_rconst(other));} 
			const FixedPt operator+(const float other) 		const { return fixedpt_add( this->value, fixedpt_rconst(other));}
			const FixedPt operator+(const int16_t other) 	const { return fixedpt_add( this->value, fixedpt_rconst(other));}



			const FixedPt operator-(const FixedPt &other) const { return fixedpt_sub( this->value, other.value);} 
			const FixedPt operator-(const fixedpt other) 	const { return fixedpt_sub( this->value, other);}
			const FixedPt operator-(const double other) 	const { return fixedpt_sub( this->value, fixedpt_rconst(other));} 
			const FixedPt operator-(const float other) 		const { return fixedpt_sub( this->value, fixedpt_rconst(other));}
			const FixedPt operator-(const int16_t other) 	const { return fixedpt_sub( this->value, fixedpt_rconst(other));}

			const FixedPt operator-(void) const {return -value;}

			const FixedPt operator*(const FixedPt &other) const { return fixedpt_mul(this->value, other.value);} 
			const FixedPt operator*(const fixedpt other) 	const { return fixedpt_mul(this->value, other);} 
			const FixedPt operator*(const double other) 	const { return fixedpt_mul(this->value, fixedpt_rconst(other));} 
			const FixedPt operator*(const float other) 		const { return fixedpt_mul(this->value, fixedpt_rconst(other));} 
			const FixedPt operator*(const int16_t other) 	const { return fixedpt_mul(this->value, fixedpt_rconst(other));} 



			const FixedPt operator/(const FixedPt &other) const { return fixedpt_div(this->value, other.value);}
			const FixedPt operator/(const fixedpt other) 	const { return fixedpt_div(this->value, other);}
			const FixedPt operator/(const double other) 	const { return fixedpt_div(this->value, fixedpt_rconst(other));} 
			const FixedPt operator/(const float other) 		const { return fixedpt_div(this->value, fixedpt_rconst(other));} 
			const FixedPt operator/(const int16_t other) 	const { return fixedpt_div(this->value, fixedpt_rconst(other));} 
